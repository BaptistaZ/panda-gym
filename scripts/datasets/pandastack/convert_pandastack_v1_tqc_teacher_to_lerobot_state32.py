...
def build_state_names() -> list[str]:
    return [f"observation_{idx}" for idx in range(32)]
...
def validate_record(record: dict[str, Any], episode_dir: Path) -> None:
    ...
    if len(record["observation"]) != 32:
        raise ValueError(f"Invalid observation length: {len(record['observation'])}. Expected 32.")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Convert a raw PandaStack-v1 TQC teacher dataset to LeRobot format using only the 32D TimeFeatureWrapper observation as state."
    )
...
def main() -> None:
    ...
    try:
        for episode_dir in episode_dirs:
            ...
            for record in records:
                ...
                state = np.asarray(record["observation"], dtype=np.float32)
    ...
    result = {
        ...
        "repo_id": args.repo_id,
        "robot_type": args.robot_type,
        "state_variant": "observation_only_32d",
        "fps": args.fps,
