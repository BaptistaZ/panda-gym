...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Live interactive SmolVLA PandaReach-v3 demo with PyBullet GUI."
    )
    ...
    parser.add_argument("--visual-substeps", type=int, default=30)
    parser.add_argument("--substep-delay", type=float, default=0.0)
...
def smooth_action(
    ...
    smoothed = float(ema) * previous_action + (1.0 - float(ema)) * raw_action
    return smoothed.astype(np.float32), smoothed.astype(np.float32)


def visual_policy_step(
    env,
    action: np.ndarray,
    visual_substeps: int,
    substep_delay: float,
...
def main() -> None:
    ...
    try:
        ...
        print("\nThe instruction was received. The robot will now execute the SmolVLA policy.")
        countdown(env, args.countdown)

        print("\nExecuting SmolVLA live policy...")
        ...
        for step in range(args.max_policy_steps):
            ...
            smoothed_action, previous_action = smooth_action(
            ...
            obs, reward, terminated, truncated, info = visual_policy_step(
                ...
                substep_delay=args.substep_delay,
            )
