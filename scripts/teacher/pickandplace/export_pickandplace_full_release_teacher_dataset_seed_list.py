...
def load_seed_list(path: Path) -> list[int]:
    """Load a deterministic list of episode seeds."""
    seeds: list[int] = []
    ...
    if not seeds:
        raise ValueError(
            f"Seed list is empty: {path}"
        )

    if len(set(seeds)) != len(seeds):
        raise ValueError(
            f"Seed list contains duplicated seeds: {path}"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional text file with one explicit seed per line. "
            "When provided, seeds are tried in file order instead "
            "of using --seed-start."
        ),
    )
...
def main() -> None:
    ...
    seed_list = (
        load_seed_list(args.seed_list)
        if args.seed_list is not None
        else None
    )

    if seed_list is not None:
        max_attempts = len(seed_list)

        if len(seed_list) < args.episodes:
            raise ValueError(
                "--seed-list must contain at least --episodes seeds"
            )
    else:
        max_attempts = (
            args.max_attempts
